...
public:
	...
	{
		Graph working( *this );  // create a copy of the graph
		...
		do
		{
			// if DJ finds a path, found is true, so compute the below if block
			found = working.BestDistance( from, to ) != INT_MAX; // changed == to !=
			if( found ) // if it does exist
			{
				int smallest = 0;
				...
				while( tracer != from )  
				{
					for( auto edge : working._mainData[tracer]->_ins ) // for every edge coming in to my "to" vertex -- max(from, to)
					{
						if( edge->_from->_data == working._mainData[tracer]->_fromWhere ) // if that edge's data is equal to where I come from
						{
	...
	int FindMinimalCover( Graph* tAnswer )  // THURS
	{
		tAnswer->Clear();					
		for( auto one : _mainData )			 // loop through main data
		{
			tAnswer->Add( one.first );		 // populate the graph
			one.second->_processed = false;  // mark each vertex as unprocessed  -- 'was _fromWhere'
		}
